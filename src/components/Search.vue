<template>
  ...
  <div class="list">
    <span @click="cl">id<Icon_triangle /></span>
  ...
  <div class="list_container">
    <div class="list_item2" v-for="item in SearchResalut">
      <span>{{ item.id }}</span>
      <span @click="show">{{ `${item.Fistname} ${item.Lastname}` }}</span>
      <span>{{ item.sex }}</span>
...
<script>
export default {
  props: ["CriminalList", "SearchInput", "SearchResalut"],
  methods: {
    cl() {
      console.log(this.SearchResalut);
    },
  },
};
</script>
...
.list_item2 {
  ...
  font-weight: 700;
  display: grid;
  grid-template-columns: 135px 277px 191px 238px 384px 211px;
  margin-left: 114px;
...
@media screen and (max-width: 1401px) {
  ...
  .list_item2 {
    grid-template-columns: 85px 227px 141px 188px 134px 160px;
    width: 980px;
    margin-left: 20px;
  }
  .search_string {
